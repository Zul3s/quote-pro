...
- **Que vérifie exactement la « suffisance » ?** — Deux choses : (1) **le « quoi »** — la nature du besoin / le type de travaux est-il identifiable ? — et (2) **les quantités chiffrables** (notamment la **surface**, et selon le cas le nombre de pièces/d'éléments). Si l'un des deux manque, la description est jugée insuffisante et le message au prospect **liste précisément ce qui manque** (ex. « précisez la surface en m² et la nature des travaux »). *(Le prompt `assess` cherche activement à récolter ces infos.)*
- **Quel modèle Ollama, combien d'appels ?** — Modèle de base **`llama3.1:8b`** (configurable dans `config/services.php`), et **deux appels distincts** : `assess` (synchrone, léger, à la soumission) puis `qualify` (asynchrone, complet, dans le job). *(Garde d'entrée rapide séparée de la qualification complète ; la fusion en un appel reste une optimisation ultérieure.)*
...
- [ ] Après traitement du job de qualification, la ligne `contact_requests` a ses colonnes peuplées : `relevance`, `project_type`, `summary`, `estimated_amount_min`, `estimated_amount_max`, `lead_quality`, `priority`, et `qualified_at` **non-null** (l'urgence n'est pas une colonne : elle reste portée par `deadline`).
...
- [ ] `lead_quality` est `complete` si `phone` **et** `postal_code` sont renseignés, sinon `incomplete` (déterministe).
- [ ] `priority` est calculée **déterministe** par la formule documentée (cf. Challenges) à partir de `relevance`, `estimation` (dominant), `deadline` et `lead_quality` : `relevance=spam`/`out_of_*` → `low` ; score `≥ 5 → high`, `2–4 → medium`, `< 2 → low` ; ex. `relevant` + estimation `high` + `deadline=immediate` + `lead_quality=complete` (4+2+1=7) → `high`.
- [ ] Si le `Qualifier` lève une exception dans le job, la ligne **reste non qualifiée** (colonnes null, `qualified_at` null) et le job est **rejoué** par la queue ; la demande n'est jamais perdue ni supprimée.
...
- [ ] `Qualifier` est un **contrat** ; `OllamaQualifier` est bindé en prod, `FakeQualifier` en test — **aucun test ne fait d'appel réseau**.
- [ ] `docs/architecture.md` documente le nouveau concern (namespace `App\Qualification`, le contrat `Qualifier`, la Rule LLM, les value objects de sortie qui ne sont **pas** des `App\Data`).
- [ ] `./vendor/bin/pest tests/Unit/ArchTest.php tests/Unit/ArchDataConstructionTest.php` passe (Action toujours sans `Illuminate\Http`, controller sans `DB`/`App\Models`, la nouvelle Rule implémente `ValidationRule`).

## Implementation TODO

### Backend — fondations qualification
- [ ] Enums `app/Enums/Relevance.php` (`relevant`/`out_of_area`/`out_of_trade`/`spam`), `app/Enums/LeadQuality.php` (`complete`/`incomplete`), `app/Enums/Priority.php` (`high`/`medium`/`low`) — via `/create-action` *(pas d'enum `Urgency` : on réutilise `Deadline`)*
- [ ] Migration `database/migrations/...add_qualification_to_contact_requests.php` : colonnes nullables `relevance`, `project_type`, `summary`, `estimated_amount_min` (int), `estimated_amount_max` (int), `lead_quality`, `priority`, `qualified_at` (timestamp), + `missing_info_acknowledged` (boolean, default false) + casts enum/datetime/bool sur le modèle `ContactRequest` *(pas de colonne `urgency` : `deadline` existe déjà)* — via `/create-action`
- [ ] Contrat + impl LLM : `app/Qualification/Contracts/Qualifier.php` (interface : `assess(string $description, ArtisanProfile $profile): SufficiencyResult` — vérifie le **« quoi »** (nature du besoin) **et** les **quantités chiffrables** (surface, nombre de pièces/éléments), `SufficiencyResult` portant `sufficient: bool` + `message` listant ce qui manque ; et `qualify(ContactRequest $request, ArtisanProfile $profile): QualificationResult`), value objects de sortie `app/Qualification/SufficiencyResult.php` + `app/Qualification/QualificationResult.php` (readonly, **pas** des `App\Data`), impl `app/Qualification/OllamaQualifier.php` (HTTP Ollama, parsing JSON), double `tests/Fakes/FakeQualifier.php` (modes configurables : sufficient/insufficient, résultat de qualif fixé, **et mode « throwing » pour simuler une panne** sur `assess`/`qualify`) — *(nouveau concern hors skill ; scaffolder à la main puis cadrer dans `docs/architecture.md`)*
- [ ] Binding explicite `Qualifier::class => OllamaQualifier::class` dans `AppServiceProvider` + section `ollama` dans `config/services.php` (`base_url`, `model` défaut `llama3.1:8b`) + tranches d'estimation pour la priorité (`estimation_high` défaut `3000`, `estimation_medium` défaut `800`)

### Backend — garde synchrone (boucle infos manquantes)
- [ ] Champ `acknowledgeMissingInfo` (bool, default false) ajouté au DTO `app/Data/SubmitContactRequestData.php` (`nullable|boolean`) + propagation de `missing_info_acknowledged` à la création — via `/create-action`
- [ ] Business rule `app/Rules/DescriptionIsSufficient.php` (`implements ValidationRule`, dépend du `Qualifier` + charge l'`ArtisanProfile`, `$fail($result->message)` quand insuffisant ; **try/catch fail-open** : toute exception du `Qualifier` est loggée et la Rule **passe** — ne bloque jamais) — via `/create-action`
- [ ] Brancher la Rule dans l'Action **existante** `app/Actions/SubmitContactRequest.php` : valider via `Validator` **avant** `ContactRequest::create`, **uniquement si** `! $data->acknowledgeMissingInfo` ; sinon persister directement avec `missing_info_acknowledged = true` — via `/create-action`
- [ ] Action tests `tests/Feature/Action/SubmitContactRequestTest.php` étendus : (a) description insuffisante sans override (FakeQualifier insufficient) → exception + rien en base ; (b) description insuffisante **avec** `acknowledgeMissingInfo` → création + `missing_info_acknowledged = true`, **sans** appel à la garde ; (c) description suffisante → création normale + `ContactRequestSubmitted` dispatché ; (d) **`Qualifier::assess` qui jette (FakeQualifier en mode throwing) → demande créée quand même** (fail-open), pas d'exception propagée — via `/create-tests-action`

### Backend — étiquetage asynchrone
- [ ] Action `app/Actions/QualifyContactRequest.php` (`handle(ContactRequest): ContactRequest` : charge le profil, appelle `Qualifier::qualify`, calcule les étiquettes déterministes qualité/priorité — la priorité lisant `deadline` directement —, persiste les colonnes + `qualified_at`) — via `/create-action`
- [ ] Listener `app/Listeners/QualifySubmittedContactRequest.php` (`implements ShouldQueue`, délègue à l'Action `QualifyContactRequest`) + wiring `ContactRequestSubmitted => [QualifySubmittedContactRequest::class]` dans `EventServiceProvider` — via `/create-action`
...
- [ ] Listener tests : le Listener est `ShouldQueue` et appelle l'Action ; un `Qualifier` qui jette laisse la ligne non qualifiée (colonnes null) — via `/create-tests-action`

### Backend — lecture
- [ ] Adapter `app/Actions/ListContactRequests.php` : tri `priority` desc puis `created_at` desc ; le payload inclut les nouvelles colonnes — via `/create-action`
